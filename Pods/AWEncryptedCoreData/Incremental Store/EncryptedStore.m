...
- (BOOL)migrateFromModel:(NSManagedObjectModel *)fromModel toModel:(NSManagedObjectModel *)toModel error:(NSError **)error {
    ...
    [self migrateManyToMany:error];
    
    return success;
}

- (BOOL)migrateManyToMany:(NSError**) error {
    //get from storeEntities to keep the original order which is important in naming the many-to-many tables
    ...
    for(int i=0; i<[entities count]; i++) {
        NSEntityDescription *desc = entities[i];
        [[desc relationshipsByName] enumerateKeysAndObjectsUsingBlock:^(NSString *key, NSRelationshipDescription *relation, BOOL *stop){
            ...
            if ([relation isToMany] && [inverse isToMany]) {
                ...
                if (![tableNames containsObject:tableName]) {
                    BOOL hasTable;
                    if ([self hasTable:&hasTable withName:tableName error:error] && !hasTable) {
...
-(NSString *)tableNameForRelationship:(NSRelationshipDescription *)relationship {    
    ...
    NSArray *relationships = @[relationship, [relationship inverseRelationship]];
    //sort the relationships so the many to many table names will be consistent
    NSArray *sortedRelationships = [relationships sortedArrayUsingComparator:[self entityRelationshipCompare]];
    int count = [sortedRelationships count];
    NSMutableArray *sortedNames = [NSMutableArray arrayWithCapacity:count];
    for(int i=0; i<count; i++){
        sortedNames[i] = [sortedRelationships[i] name];
    }
    return [NSString stringWithFormat:@"ecd_%@",[sortedNames componentsJoinedByString:@"_"]];
}

- (NSComparator)entityRelationshipCompare
{
    NSArray *entities = [self storeEntities];
    int count = [entities count];
    NSMutableArray *entityNames = [NSMutableArray arrayWithCapacity:count];
    for(int i=0; i<count; i++){
        entityNames[i] = [entities[i] name];
    }

    return ^NSComparisonResult(NSRelationshipDescription *rel1, NSRelationshipDescription *rel2) {
        NSString *name1 = [[rel1 entity] name];
        NSString *name2 = [[rel2 entity] name];
        return [entityNames indexOfObject:name1] - [entityNames indexOfObject:name2];
    };
}
